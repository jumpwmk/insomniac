...
class TestrunController extends Controller {
	...
	function initContestantData($contestant)
	{
		if($contestant->data != '')
		{
			...
			if($contestant->data->type != "testrun")
			{
				$contestant = null;
				exit(0);
			}
	...
	function getResult($submit, $contest, $result_type = "full")
	{
		...
		TestrunController::initTaskData($task);
		...
		if($check == 1)
		{
			...
			foreach ($task->data->group as $key => $group) {
				...
				foreach ($group->case as $sub_key => $case) {
					...
					/*if($submit->result[$case - 1] == 'P') $tmp .= 'K';
					else if($submit->result[$case - 1] == '-') $tmp .= '0';
					else if($submit->result[$case - 1] == 'X') $tmp .= 'E';
					else $tmp .= $submit->result[$case - 1];*/
			...
			if($result_type == "full") $submit->result = $tmp;
	...
	public function getTasks()
	{
		...
		TestrunController::initContestData($contest);
		...
		if(strtotime($contest->start_contest) <= time() or Auth::isAdmin())
		{
			...
			foreach ($tasks as $key => $task) {
				...
				foreach ($users as $key => $user) {
					...
					TestrunController::initContestantData($user);
					...
					if(isset($user->data->submit_of_task->$tmp))
					{
						...
						{
							if(Auth::isAdmin() or strtotime($contest->end_contest) <= time() or !$contest->data->score_only)
								TestrunController::getResult($submit, $contest, "full");
							else
								TestrunController::getResult($submit, $contest, "score_only");
		...
		if(strtotime($contest->start_contest) <= time() or Auth::isAdmin())
		{
			...
			foreach ($submits as $key => $value)
			{
				...
				else if(Queue::where('submit_id','=',$value->id)->count())
				{
					...
					TestrunController::getResult($submits[$key], $contest, "full");
				else if(strtotime($contest->end_contest) <= time() or !$contest->data->score_only)
					TestrunController::getResult($submits[$key], $contest, "full");
				else
				{
					TestrunController::getResult($submits[$key], $contest, "score_only");
			...
			if(in_array($code->getMimeType(), $type))
			{
				...
				if(Auth::isAdmin() or ($contest->start_contest <= time() and time() <= $contest->end_contest and Contestant::whereRaw("user_id = ? and contest_id = ?", [Auth::user()->id, $contest->id])->count()))
				{
					...
					if(Contestant::whereRaw("user_id = ? and contest_id = ?", [Auth::user()->id, $contest->id])->count())
					{
						...
						TestrunController::initContestantData($contestant);
						$contestant->data->submit_of_task->$post['task_id'] = $submit->id;
						$contestant->data = json_encode($contestant->data);
						$contestant->save();
					}

					return redirect('contest/testrun/'.$post['contest_id'].'/result');
				}
			}
		}
		return redirect('contest/testrun/'.$post['contest_id'].'/task/'.$post['task_order']);
	}
