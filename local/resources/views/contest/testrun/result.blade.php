...
Contest::isTrueType($contest_id, 'testrun'); 
?>
<html ng-app="mainApp">
	<head>
	...
	<body ng-cloak>
		...
		<div class="container" ng-controller="MainController" ng-init="realTime(<?= time() ?>)">
			...
			<div class="row" ng-controller="TestrunController as ConCtrl" ng-init="ConCtrl.getContest('{{$contest_id}}');">
				<div class="col-md-8">
							...
							<table class="table table-condensed table-hover" style="text-align:center">
								<thead>
									<tr>
										...
										<td ng-show="ConCtrl.contest.data.score_only == '0' || ConCtrl.contest.status == 'old'"><b><a href ng-click="key = 'result'; rev = !rev">ผลตรวจ</a></b></td>
										<td><b><a href ng-click="key = 'score'; rev = !rev">คะแนน</a></b></td>
										<td ng-show="ConCtrl.contest.data.score_only == '0' || ConCtrl.contest.status == 'old'"><b><a href ng-click="key = 'time'; rev = !rev">เวลา</a></b></td>
										<td ng-show="ConCtrl.contest.data.score_only == '0' || ConCtrl.contest.status == 'old'" style="width:120px"><b><a href ng-click="key = 'memory'; rev = !rev" style="width:120px">หน่วยความจำ</a></b></td>
									</tr>
								</thead>
								<tbody>
									<tr ng-repeat="submit in ConCtrl.submits | orderBy: key: rev" ng-class="{'success': submit.pass == 1}" ng-show="submit.visible == '1'">
										...
										<td ng-show="ConCtrl.contest.data.score_only == '0' || ConCtrl.contest.status == 'old'">
											<span ng-show="submit.result.length">@{{submit.result}}</span>
											<span ng-hide="submit.result.length || submit.result.compile_result.length"><a href ng-click='ConCtrl.setShowError(submit.compile_result)' data-toggle="modal" data-target="#showError">compilation error</a></span>
										</td>
										
										<td>
											<!-- <span ng-show="ConCtrl.contest.data.score_only == '1' && ConCtrl.contest.status != 'old' && submit.pass == null">@{{submit.result}}</span> -->
											@{{(submit.score >= 0) ? submit.score : submit.result}}
											<a href ng-click='ConCtrl.setShowError(submit.compile_result)' ng-show="submit.compile_result.length" data-toggle="modal" data-target="#showError"><i class='fa fa-exclamation-triangle'></i></a></span>
										</td>
										<td ng-show="ConCtrl.contest.data.score_only == '0' || ConCtrl.contest.status == 'old'">@{{submit.time}} s</td>
										<td ng-show="ConCtrl.contest.data.score_only == '0' || ConCtrl.contest.status == 'old'">@{{submit.memory}} KB</td>
									</tr>
								</tbody>
							</table>
								...
								<div class="modal-dialog" style="width:50%">
									<div class="modal-content">
										<div class="modal-header">
											...
											<h4 class="modal-title">Compilation message</h4>
										</div>
