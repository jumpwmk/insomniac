...
<html ng-app="mainApp">
	...
	<body ng-cloak>
		...
		<div class="container" ng-controller="MainController" ng-init="realTime(<?= time() ?>)">
			...
			<div class="row" ng-controller="PartialFeedbackController as ConCtrl" ng-init="ConCtrl.getContest('{{$contest_id}}'); ConCtrl.getTasks('{{$contest_id}}')">
				<div class="col-md-8">
					...
					<div>
					@else
					<!-- Show if contest ended and pending system test-->
					<div ng-show="ConCtrl.contest.status == 'old' && ConCtrl.contest.data.scoreboard_pending == '1'">
						<br><br>
						<h1 style="color:#999; text-align:center">กำลังทำการตรวจด้วยข้อมูลทดสอบจริง <br>(Pending system test)</h1>
					</div>
					<!-- Show if (not ended and scoreboard is not allowed) and not showing above-->
					<div ng-show="(ConCtrl.contest.status != 'old' && ConCtrl.contest.data.scoreboard == '0') && !(ConCtrl.contest.status == 'old' && ConCtrl.contest.data.scoreboard_pending == '1')">
					...
					<!-- Show if ended or show scoreboard is allowed but not when pending -->
					<div ng-show="(ConCtrl.contest.status == 'old' || ConCtrl.contest.data.scoreboard == '1') && !(ConCtrl.contest.status == 'old' && ConCtrl.contest.data.scoreboard_pending == '1')">
					@endif
