...
<html ng-app="mainApp">
	...
	<body ng-cloak>
		...
		<div class="container" ng-controller="MainController" ng-init="realTime(<?= time() ?>)">
			...
			<div class="row" ng-controller="ACMContestController as ConCtrl" ng-init="ConCtrl.getContest('{{$contest_id}}'); ConCtrl.getTasks('{{$contest_id}}')">
				<div class="col-md-8">
					...
					@endif
						<div class="form-horizontal">
							...
							<table class="table table-condensed" ng-init="ConCtrl.getContestants('{{$contest_id}}', false)" style="text-align:center">
								...
								<tbody>
									<tr ng-repeat="user in ConCtrl.contestants | filter: searchUsers | orderBy: 'place'" ng-class="{'active': user.me == '1'}">
										<td>@{{user.place}}</td>
										...
										<td ng-repeat="task in ConCtrl.tasks | orderBy: 'order'" ng-class="{'success': user.pass[task.order] == '1', 'danger': user.pass[task.order] == '0'}">
											<span ng-show="user.pass[task.order] != null">@{{user.submission_count[task.order]}} / @{{user.penalty_time[task.order]}}</span>
										</td>
										<td><b>@{{user.total_penalty}}</b></td>
